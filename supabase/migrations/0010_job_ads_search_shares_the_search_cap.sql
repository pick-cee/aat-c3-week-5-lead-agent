-- Both paid searches share one guard. search_job_ads (LinkedIn job ads, for
-- "hiring for a role" briefs) is a paid Apify call like search_companies, so
-- it counts toward max_apify_calls and the one-search-per-step rule, and a
-- step may use one or the other, never both. Without this the new tool would
-- have been counted only as a generic tool call.
...
declare
  ...
  v_is_apify boolean := right(p_tool_name, length('search_companies')) = 'search_companies'
    or right(p_tool_name, length('search_job_ads')) = 'search_job_ads';
